var RadarChart = {
  draw: function(id, d, options){
    ...
    var container = typeof id === "string" ? d3.select(id) : id;
    ...
    var g = container.append("svg").attr("width", cfg.w).attr("height", cfg.h).append("g");
    ...
    d.forEach(function(y, x){
      ...
      g.selectAll(".area")
                     ...
                     }).transition().duration(1500).attr("points",function(d) {
                         var str="";
                         for(var pti=0;pti<d.length;pti++){
    ...
    d.forEach(function(y, x){
      ...
      circle.attr('r', cfg.radius)
        ...
        .transition().duration(1500).attr("cx", function(j, i){
	          dataValues.push([
	            cfg.w/2*(1-(parseFloat(Math.max(j.value, 0))/cfg.maxValue)*cfg.factor*Math.sin(i*cfg.radians/total)), 
	            cfg.h/2*(1-(parseFloat(Math.max(j.value, 0))/cfg.maxValue)*cfg.factor*Math.cos(i*cfg.radians/total))
	        ]);
